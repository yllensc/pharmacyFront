...
async function loadMedicinesWithRecipes() {
  try {
    const response = await fetch(end4, options);
    if (!response.ok) {
      throw new Error(`Failed. State: ${response.status}`);
    }
    const result = await response.json();
    if (result != "") {
      result.forEach((medicine) => {
        const { saleId, medicineName, quantity } = medicine;
        ...
        $tableEnd4.insertAdjacentHTML("beforeend", html);
      });
